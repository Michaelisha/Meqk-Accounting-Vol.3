...
export default function CashReportPage() {
  ...
  return (
    <DepartmentLayout theme="finance" title="Cash Entry Report" navigation={FINANCE_NAVIGATION}>
              ...
              <tbody className="divide-y divide-slate-50">
                {data.map((row, i) => (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-8 py-5 text-sm font-black text-slate-900 italic">{row.accounts?.name}</td>
                    <td className="px-8 py-5 text-sm font-bold text-slate-600">{row.description}</td>
                    <td className="px-8 py-5 text-sm font-black text-slate-900 text-right">TZS {row.amount.toLocaleString()}</td>
                  </tr>
                ))}
                {data.length === 0 && !loading && (
                  <tr>
                    <td colSpan={4} className="px-8 py-20 text-center text-slate-400 italic">No cash entries found.</td>
                  </tr>
                )}
              </tbody>
            </table>
